...
class robot:
    ...
    def sense(self):
        ...
        for i,j in enumerate(measurements):
            if abs(j[1]) < self.measurement_range and abs(j[2]) < self.measurement_range:
                s.append(j)
    ...
    def make_landmarks_non_rand(self, num_landmarks):
        self.landmarks = []
        for y in range(50,80,5):
            self.landmarks.append([30,y])
        for x in range(30,80,5):
            self.landmarks.append([x,80])
        for y in range(30,80,5):
            self.landmarks.append([80,y])
